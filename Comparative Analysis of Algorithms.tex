...
\title{Comparative Analysis of Algorithms for Unique Username Generation
and Verification at Scale: A Simulation-Based Benchmark Study}
...
A variety of algorithms have been proposed in literature to address these issues, however, these algorithms are mostly investigated individually and under specific circumstances and a single ``best'' username system algorithm to address a high-volume of users is difficult to pinpoint. In this study, six usernames generation techniques - Random Suffix Generation, Trie-based Suggestion, Snowflake-like Suffix Generation, Agentic Username Generation, Bloom Filter and Consistent Hashing are evaluated using varying username records sizes from 10,000 to 1,000,000. Tests were performed on a range of metrics including latency, memory usage, uniqueness, usability and distribution cost when generating and distributing username accounts across many distributed servers. Important observations include: the Bloom filter takes up 41$\times$ less memory than a basic hash set,
as well as 923$\times$ less than the non-compressed trie for
1,000,000 usernames at the expense of a managed false-positive rate
at around its target of 1\%; consistent hashing re-maps only 11.4--11.7\%
of keys in response to adding a new node, compared to 88.4--88.8\%
for naive modulo hashing; and the latency of suggestion by tries scales
about 178$\times$ upon a 100$\times$ increase in corpus size because of
an unoptimized prefix counting function. The key drawback of this study
is that the agentic approach is an implementation of a deterministic
rule-based simulator, rather than a real-life large language model, and
thus its outcomes should be treated as a conservative estimate, rather
than actual measurements of the LLM-based generation process.
This paper's novelty does not lie in any individual algorithms but
rather in a common, task-aware testing framework and a reproducible benchmark that allows practitioners to choose proper combinations of
generation and verification methods according to the scale of their deployment,
not just according to the performance of each algorithm separately.
\end{abstract}

\begin{IEEEkeywords}
username generation, username verification, Bloom filter, consistent
hashing, trie, distributed systems, collision rate, false-positive
rate, scalability benchmarking
\end{IEEEkeywords}
...
To ensure a fair comparison, this paper relies upon a single multi-dimensional evaluation framework. The framework is based strictly on the necessities of username registration systems discovered during the comprehensive review. The evaluated criteria include query latency, memory consumption, collision rate and how easy it is for humans to remember and read generated usernames.
They were chosen since each of them relates to some cost incurred by a real-world registration system. Both mean and 95th percentile latencies were chosen since they define how responsive the registration process will feel to
a user; mean latency is responsible for normal-case costs, while
p95 latency highlights the nearly worst-case performance that
can be missed by mean latency due to a possibility of one
slow outlier to impact a user experience when registration
peaks. Memory usage was chosen as the major cost of
infrastructure needed for verification structures, especially
for those that need to be stored in memory in several
servers. Success rate was chosen since the generation
process that fails to create a suitable name often enough is
not practical no matter how efficient in other ways it is. Collision rate was selected because it is the most direct
measurement of how effectively an algorithm avoids the actual
problem this paper seeks to address, that of name collision. It is defined differently for verification
(a false positive) and for generation (a candidate name that is found
to already exist), as the two different tests examine different
fundamental operations. Readability was selected because an
algorithmically unique, but difficult to read or memorize, username, such
as a random alphanumeric string, is imposing a cost on the user despite
perfectly satisfying the uniqueness requirement; there is no other
algorithmic measurement that addresses this cost. Distributed deployment
was selected because username systems are increasingly distributed across
multiple machines or datacenters, and a measure of a technique's
distributed-deployment efficiency, specifically how much state needs to be
recreated when scaled out to a cluster, does not emerge at all from a
single-machine test.

Finally, it evaluates whether a technique is suitable for deployment across multiple sites. The six techniques under consideration are random suffix generation, Trie-based suggestion, Snowflake-like suffix generation, agentic username generation, Bloom filter pre-filtering, and consistent hashing. Each technique is treated as a black-box module within a username registration system. It can perform one of two functions. The first is the \textit{generation} function, which proposes a candidate username. The second is the \textit{verification/distribution} function, which determines whether the proposed username is available or not. 

Since these six techniques have different functions, an evaluation of the
techniques based on one undifferentiated scale will be deceptive;
Table~\ref{tab:taskmatrix} highlights which function each of these
techniques serves and the evaluation criteria relevant to its
functioning. A technique can be judged only on those criteria which are
relevant to the output produced by the technique; criteria which are
not relevant for the function performed by a particular technique will
be deemed as non-applicable.

\begin{table}[h]
\centering
\caption{Task-specific evaluation matrix}
\label{tab:taskmatrix}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
Method & Role & Latency & Memory & Collision & Readability \\
\midrule
Random suffix     & Generation & \checkmark & n/a & \checkmark & \checkmark \\
Snowflake-style   & Generation & \checkmark & n/a & \checkmark & \checkmark \\
Trie suggestion   & Generation & \checkmark & \checkmark & \checkmark & \checkmark \\
Agentic (rule-based) & Generation & \checkmark & n/a & n/a & \checkmark \\
Bloom filter      & Verification & \checkmark & \checkmark & \checkmark (FP rate) & n/a \\
Consistent hashing & Distribution & \checkmark & n/a & n/a & n/a \\
\bottomrule
\end{tabular}%
}
\end{table}

This table makes clear that the six techniques are not all interchangeable:
the bloom filter and consistent hashing are used to perform the
verification/distribution function and can be combined naturally in
an industrial pipeline process, whereas the four methods for
generation of names are competing alternatives for the distinct job
of proposing an eligible name. This comparison in this paper is
therefore not “which one technique is better” but rather
“which technique is better for its function, and which pair of
functions is consistent,” consistent with the workload-dependent
recommendation made in the Discussion.

In distributed environments, this function also identifies the node responsible for handling the verification request. The strength of this approach is based on that, in reality, username registration systems are not dependent on a single approach. Usually, they use a username generation method coupled with a verification method. Because of this, assessing both kinds under a common set of metrics makes for a more accurate comparison. Such a comparison allows one to detect not only the fastest method at a time but also the best combinations of methods for different deployment scenarios.
...
The two datasets of 10,000 and 100,000 usernames were used directly in the experiment. While, the 1,000,000 username dataset was used in two ways: in its entirety and as a source of sub-datasets of 50,000, 100,000, and 250,000 usernames. The latter was done by extracting the first usernames from the large dataset. The use of both the actual datasets and the sub-datasets from the largest dataset enabled the verification performance to be tested against increasing numbers of usernames.
...
and non-deterministic outputs. Thus, the results achieved in this case with the agentic style approach need to be considered as an estimate of what such a language model might be able to achieve. Certainly, a real language model could offer a number of more appropriate
suggestions for usernames than any fixed set of rules, but this was not
verified in this current research. Thus, any statement made in this paper regarding the readability, latency or other performance metrics of the
``agentic'' approach must not be interpreted as applying specifically to large language models, since any such findings are only specific to this
study’s deterministic and rule-based implementation of one piece of Nominalist methodology \cite{b6}.
...
In summary, this approach, comprising the six applied methods, along with the common evaluation framework and the practical datasets used, is intended to tackle directly the issues of collision and squatting of usernames encountered in practice \cite{b9}.
...
\caption{Peak memory consumption vs. corpus scale.}
\label{fig:memory}
\end{figure}

The average query latency for each structure is depicted in Figure~\ref{fig:latency}. The plain set gave the lowest query latencies (0.23--0.41$\mu$s) among all scales tested, while the trie structure gave an intermediate level (0.95--3.52 $\mu$s), and the Bloom filter had the largest (2.96--4.62 $\mu$s). The reason behind this is that the queries to the Bloom filter involve the calculation of multiple hash values, not just the dictionary lookups. This is the opposite order to the one in the memory requirements: the structure taking the least amount of memory space has the highest query overhead.
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/latency_vs_scale.png}
\caption{Average query latency vs. corpus scale.}
\label{fig:latency}
\end{figure}
...
\caption{Bloom filter false-positive rate vs. corpus scale.}
\label{fig:fpr}
\end{figure}
...
\caption{Key remapping fraction: consistent hashing vs. naive modulo.}
\label{fig:remap}
\end{figure}
...
based approximation of the agentic method performed moderately slower than the above methods averaging between 46--58 $\mu$s across all three corpus sizes due to its evaluation process of a fixed size candidate set that is not scale dependent. On the other hand, trie-based suggestion showed a much steeper
...
When all of these results are combined, it becomes evident that a workload-dependent approach should be recommended, rather than one optimal technique. In other words, a production registration system that seeks to ensure efficient performance with low memory usage. With regard to verification and routing, the findings suggest that Bloom filters and consistent hashing are the best options in terms of memory efficiency,scalability, and lookup performance. Username generation can involve the use of a fast and collision-resistant algorithm like the Snowflake algorithm suffix, along with a readable algorithm for enhanced quality of the username generated. Regarding the readability aspect, the findings of this research
directly confirm the validity of the rule-based approach being considered;
whether an actual LLM-based one would yield even better results
remains an open question based on the Nominalist design \cite{b6},
but it cannot be confirmed due to the lack of actual LLM use. Trie-based proposals should be used only when working with small corpora due to increased memory usage.
...
\bibitem{b9} A. Lepipas, A. Borzoo, E. De Cristofaro, and G. Stringhini,
``Username Squatting on Online Social Networks: A Study on X,'' in
\textit{Proc. 19th ACM Asia Conf. on Computer and Communications
Security (ASIA CCS '24)}, Singapore, Jul. 2024, pp. 269--283,
