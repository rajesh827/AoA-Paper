...
The primary variable considered in this study is the corpus scale, defined as the number of existing usernames in the system. Techniques of verification and distributed-deployment were tested with 5,000, 20,000, 50,000, and 100,000 usernames to investigate their performance with respect to changing corpus sizes. Generation techniques were tested with 100,000 and 500,000 usernames. These two corpus sizes proved to be enough to detect any performance trends without taking the time to test four corpus sizes.
...
Methods that do not generate a value in their natural form for the given dimension are not applicable for that dimension, rather than generating an artificial value. The agentic style of generation method, for example, does not try sequentially like the other three generation methods; it tries a batch of ten rule-based candidate generations and takes the best candidate it finds, so first-guess collision rate is not a relevant measure and is therefore listed as such, rather than forcing it into the same column as the three sequential retry methods.
